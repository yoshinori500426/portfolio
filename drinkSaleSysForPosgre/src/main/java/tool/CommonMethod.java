...
public class CommonMethod {
	...
	// ②ログイン状態確認
	// 戻り値 なし
	public void actionBefProc(HttpServletRequest request, HttpServletResponse response) throws IOException {
	...
	// 画面遷移用メソッド(クラス｢InputOutputScreen｣中で使用)
	// 戻り値 なし
	public void dispatchToAction(String toAction, HttpServletRequest request, HttpServletResponse response)
			throws IOException {
		HttpSession session = request.getSession();
		PrintWriter out = response.getWriter();
		try {
	...
	// 画面遷移用メソッド(クラス｢InputOutputScreen｣を他クラスで使用)
	// 戻り値 なし
	public void dispatchToActionByInputOutputScreenClass(String string, HttpServletRequest request,
			HttpServletResponse response) throws IOException {
		HttpSession session = request.getSession();
		PrintWriter out = response.getWriter();
		try {
			String toAction = string;
			session.setAttribute("toAction", toAction);
			new InputOutputScreen().doPost(request, response);
		} catch (Exception e) {
			e.printStackTrace(out);
		}
	}
	...
	public boolean janCodeCheck(HttpServletRequest request, HttpServletResponse response, String reqParaName,
		...
		// 入力値チェック(正規表現を使用)
		boolean judge = new CommonMethod().inputValueCheck(request, response, reqParaName, regEx);
	...
	// 戻り値は､値欄の記述が妥当であれば｢true｣､そうでない場合｢false｣
	public boolean quantityCheck(HttpServletRequest request, HttpServletResponse response, String reqParaName,
		...
		// 入力値チェック(正規表現を使用)
		boolean judge = new CommonMethod().inputValueCheck(request, response, reqParaName, "^\\d+$");
		...
		if (judge == false && !reqPara.equals("")) {
			...
			int result = new CommonMethod().setAlart(request, response, alartNum);
			if (result == 0) {
				session.setAttribute("state", "１以上の整数以外の値が入力されています｡\\n入力内容をお確かめ下さい｡");
			} else {
				session.setAttribute("state", "アラートセットが異常終了しました｡");
			}
		} else if (judge == true) {
			// 入力値チェック(正規表現を使用)
			judge = new CommonMethod().inputValueCheck(request, response, reqParaName, ".{1,9}");
			if (judge == false) {
				int[] alartNum = new int[10];
				alartNum[alartNo - 1] = alartNo;
				// 配列｢int[] alartNum｣の値を基に､配列｢String[] alart｣に値をセット
				...
				int result = new CommonMethod().setAlart(request, response, alartNum);
				if (result == 0) {
					session.setAttribute("state", "入力上限を超えた値が入力されています｡\\n入力内容をお確かめ下さい｡");
				} else {
					session.setAttribute("state", "アラートセットが異常終了しました｡");
				}
			} else {
	...
	public boolean blankCheck(HttpServletRequest request, HttpServletResponse response, String reqParaName,
		...
		if (judge == false) {
			...
			int result = new CommonMethod().setAlart(request, response, alartNum);
			if (result == 0) {
				session.setAttribute("state", "規定外の文字数です。\\n入力内容をお確かめ下さい｡");
			} else {
				session.setAttribute("state", "アラートセットが異常終了しました｡");
			}
		}
